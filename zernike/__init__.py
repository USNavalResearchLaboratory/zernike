...
########################################################
# Convert Zernike Coefficients to Meaningful Quanities #
########################################################
...
#########################################################
# Interface with SeaRay                                 #
# https://github.com/USNavalResearchLaboratory/SeaRay   #
#########################################################
def show_key_parameters():
    '''
    Extracts useful parameters from SeaRay input file inpuy.py (Untested)
    '''
...
def extract_phase(basename,dims=False):
    '''
    Extract phase from SeaRay output. (Untested)

    basename    base of filename for SeaRay data
    dims        dimensions of array of rays 
    ''' 
...
def CalculateFocalLength(basename,wavelength, radius, dims=False,nmax=6):
    '''
    Calculate Focal Length of SeaRay Data (Untested)
...
def convert_raw_dat_to_phase(data,dims=False):
    '''
    Calculate phase from already imported SeaRay data. (Untested)

    data        dictionary of ray trajectories and ray phases.
    dims        dimensions of array of rays 
    ''' 
